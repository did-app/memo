...
<main class="w-full mx-auto md:max-w-3xl px-1 md:px-2 py-6 ">
  ...
  <form class="shadow-md max-w-2xl mx-auto w-full bg-white rounded-lg px-8 pt-4 pb-8 my-6"
  ...
  >
    ...
    <input class="w-full max-w-md block border-gray-500 shadow-md focus:border-green-600 outline-none focus:bg-gray-100 border-2 mb-6 py-2 px-4 rounded mt-4" type="text" bind:value={groupName} required placeholder="e.g. Dev Team"/>   
    <style>
    ...
    <ul class="mb-2 mt-4 pb-4">
      {#each personalContacts(inbox) as { id, emailAddress }}
        <li class="flex items-center text-gray-600 hover:text-gray-800 my-2">
          <input class="w-4 h-4 checked" type="checkbox" bind:group={invitees} value={id} />
